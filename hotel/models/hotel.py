...
class ProductCategory(models.Model):
    ...
    isroomtype = fields.Boolean('Is Hall Type')
...
class FolioRoomLine(models.Model):
    ...
    room_id = fields.Many2one(comodel_name='hotel.room', string='Hall id')
    check_in = fields.Datetime('Date From', required=True)
    check_out = fields.Datetime('Date To', required=True)
...
class HotelRoom(models.Model):
    ...
    floor_id = fields.Many2one('hotel.floor', 'Floor No',
                               help='At which floor the hall is located.')
    max_adult = fields.Integer('Max Adult')
    max_child = fields.Integer('Max Child')
    categ_id = fields.Many2one('hotel.room.type', string='Hall Category')
    room_amenities = fields.Many2many('hotel.room.amenities', 'temp_tab',
                                      'room_amenities', 'rcateg_id',
                                      string='Hall Amenities',
                                      help='List of hall amenities. ')
    status = fields.Selection([('available', 'Available'),
                               ('occupied', 'Occupied')],
                              'Status', default='available')
    capacity = fields.Integer('Capacity')
    room_line_ids = fields.One2many('folio.room.line', 'room_id',
                                    string='hall Reservation Line')
...
class HotelFolio(models.Model):
    ...
    checkin_date = fields.Datetime('Date From', required=True, readonly=True,
                                   states={'draft': [('readonly', False)]},
                                   default=_get_checkin_date)
    checkout_date = fields.Datetime('Date To', required=True, readonly=True,
                                    states={'draft': [('readonly', False)]},
    ...
    room_lines = fields.One2many('hotel.folio.line', 'folio_id',
                                 ...
                                 states={'draft': [('readonly', False)],
                                         'sent': [('readonly', False)]},
                                 help="Auditorium Hall reservation detail.")
    ...
    def go_to_currency_exchange(self):
        ...
        for rec in self:
            if rec.partner_id.id and len(rec.room_lines) != 0:
                ...
                raise except_orm(_('Warning'), _('Please Reserve Any Hall.'))
    ...
    def folio_room_lines(self):
        ...
        for room in self[0].room_lines:
            if room.room_no.id in folio_rooms:
                raise ValidationError(_('You Cannot Take Same Hall Twice'))
    ...
    def check_dates(self):
        ...
        if self.checkin_date and self.checkout_date:
            if self.checkin_date >= self.checkout_date:
                raise ValidationError(_('Booking Date Should be \
...
class HotelFolioLine(models.Model):
    ...
    order_line_id = fields.Many2one('sale.order.line', string='Order Line',
                                    required=True, delegate=True,
                                    ondelete='cascade')
    room_no = fields.Many2one('hotel.room', string='Hall No.', required=True)
    folio_id = fields.Many2one('hotel.folio', string='Folio',
                               ondelete='cascade')
    checkin_date = fields.Datetime('Date From', required=True,
                                   default=_get_checkin_date)
    checkout_date = fields.Datetime('Date To', required=True,
                                    default=_get_checkout_date)
...
class CurrencyExchangeRate(models.Model):
    ...
    today_date = fields.Datetime('Date Booked',
                                 required=True,
                                 default=(lambda *a:
                                          time.strftime
    ...
    guest_name = fields.Many2one('res.partner', string='Customer Name')
    room_number = fields.Char(string='Hall Number')
