...
class HotelReservation(models.Model):
    ...
    date_order = fields.Datetime('Date Booked', required=True, readonly=True,
                                 states={'draft': [('readonly', False)]},
                                 default=(lambda *a:
                                          time.strftime
                                          (DEFAULT_SERVER_DATETIME_FORMAT)))
    warehouse_id = fields.Many2one('stock.warehouse', 'Auditorium', readonly=True,
    ...
    partner_invoice_id = fields.Many2one('res.partner', 'Invoice Address',
                                         ...
                                         "current reservation.")
    partner_order_id = fields.Many2one('res.partner', 'Customer Contact',
                                       readonly=True,
                                       states={'draft':
                                               [('readonly', False)]},
                                       ...
                                       "or quotation.")
    partner_shipping_id = fields.Many2one('res.partner', 'Customer Address',
                                          readonly=True,
                                          states={'draft':
                                                  [('readonly', False)]},
                                          help="Customer address"
                                          "for current reservation. ")
    checkin = fields.Datetime('Date From', required=True,
                              readonly=True,
                              states={'draft': [('readonly', False)]})
    checkout = fields.Datetime('Date To', required=True,
                               readonly=True,
                               states={'draft': [('readonly', False)]})
...
class HotelRoomReservationLine(models.Model):
    ...
    state = fields.Selection([('assigned', 'Assigned'),
                              ('unassigned', 'Unassigned')], 'Hall Status')
...
class RoomReservationSummary(models.Model):
    ...
    room_summary = fields.Text('Summary')
...
class QuickRoomReservation(models.TransientModel):
    ...
    check_in = fields.Datetime('Date From', required=True)
    check_out = fields.Datetime('Date To', required=True)
    room_id = fields.Many2one('hotel.room', 'Hall', required=True)
    warehouse_id = fields.Many2one('stock.warehouse', 'Auditorium', required=True)
    pricelist_id = fields.Many2one('product.pricelist', 'pricelist',
                                   required=True)
    partner_invoice_id = fields.Many2one('res.partner', 'Customer Address',
                                         required=True)
    partner_order_id = fields.Many2one('res.partner', 'Customer Contact',
                                       required=True)
    partner_shipping_id = fields.Many2one('res.partner', 'Customer Address',
